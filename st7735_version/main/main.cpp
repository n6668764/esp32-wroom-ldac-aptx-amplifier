...
// Bluetooth amplifier build: A2DP -> decoder -> PCM5102, with a low-priority
// ST7735 spectrum/status display and four local control buttons.
static constexpr char TAG[] = "BT_AMP";
static constexpr char DEVICE_NAME[] = "ESP32 aptX Amplifier";
static constexpr char FIRMWARE_REV[] = "ldac-st7735-buttons-v1";

static constexpr gpio_num_t I2S_BCK = BOARD_I2S_BCK;
static constexpr gpio_num_t I2S_WS = BOARD_I2S_WS;
static constexpr gpio_num_t I2S_DOUT = BOARD_I2S_DOUT;
...
static volatile bool s_avrc_controller_connected;
static volatile bool s_volume_notification_pending;
...
static uint8_t s_avrc_transaction_label;

static void apply_absolute_volume(uint8_t volume, bool notify_phone)
{
    ...
    if (notify_phone && s_volume_notification_pending) {
        esp_avrc_rn_param_t response = {};
        response.volume = volume;
        if (esp_avrc_tg_send_rn_rsp(ESP_AVRC_RN_VOLUME_CHANGE,
                                    ESP_AVRC_RN_RSP_CHANGED, &response) == ESP_OK) {
            s_volume_notification_pending = false;
        }
    }
...
static esp_audio_err_t decoder_open(const esp_a2d_mcc_t *mcc)
{
    ...
    if (ldac_mcc(mcc, &ldac_rate)) {
        ...
        if (s_ldac_decoder != nullptr && ldacdecInit(s_ldac_decoder) == 0 &&
            ...
            spectrum_display_set_codec("LDAC", s_sample_rate);
        ...
        if (s_aptx_decoder != nullptr && set_i2s_rate(aptx_rate) == ESP_OK) {
            ...
            s_channels = 2;
            result = ESP_AUDIO_ERR_OK;
            spectrum_display_set_codec(aptx_hd ? "aptX HD" : "aptX Classic", s_sample_rate);
        ...
        if (result == ESP_AUDIO_ERR_OK) {
            if (set_i2s_rate(I2S_RATE) == ESP_OK) {
                ...
                spectrum_display_set_codec("SBC", s_sample_rate);
        ...
        if (result == ESP_AUDIO_ERR_OK) {
            if (set_i2s_rate(config.sample_rate) == ESP_OK) {
                ...
                spectrum_display_set_codec("AAC", s_sample_rate);
...
static void write_pcm(uint8_t *data, size_t size, uint8_t channels, uint8_t bits)
{
    ...
    static uint32_t logged_generation = UINT32_MAX;
    if (logged_generation != s_decoder_generation) {
        ESP_LOGI(TAG, "PCM output started: %u Hz, %u-bit, input channels=%u, frames=%u, speaker spatializer=%s",
                 (unsigned)s_sample_rate, (unsigned)bits, (unsigned)channels,
                 (unsigned)frames,
                 (s_sample_rate == I2S_RATE_48K || s_sample_rate == I2S_RATE_96K) ?
                     "on" : "bypass");
        logged_generation = s_decoder_generation;
    }

    // Only genuinely decoded 48/96 kHz PCM is rendered through this filter.
    // Other negotiated sample rates retain their original stereo image.
    speaker_spatializer_process(pcm, frames, s_sample_rate, s_absolute_volume);
...
static void a2dp_callback(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
    if (event == ESP_A2D_PROF_STATE_EVT &&
        ...
        if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
            s_connected = true;
            spectrum_display_set_connected(true);
            ESP_LOGI(TAG, "Bluetooth connected");
        } else if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_DISCONNECTED) {
            s_connected = false;
            s_playing = false;
            decoder_close();
            spectrum_display_set_connected(false);
            spectrum_display_set_codec(nullptr, 0);
        ...
        s_playing = playing;
        ESP_LOGI(TAG, "audio state=%d (%s)", (int)param->audio_stat.state,
                 playing ? "started" : "stopped/suspended");
...
static void avrc_target_callback(esp_avrc_tg_cb_event_t event, esp_avrc_tg_cb_param_t *param)
{
    if (event == ESP_AVRC_TG_SET_ABSOLUTE_VOLUME_CMD_EVT) {
        ...
        apply_absolute_volume(volume, false);
        ...
        s_volume_notification_pending = true;
    }
}
...
static void avrc_controller_callback(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param)
{
    if (event == ESP_AVRC_CT_CONNECTION_STATE_EVT) {
        s_avrc_controller_connected = param->conn_stat.connected;
        ESP_LOGI(TAG, "AVRCP controller %s", param->conn_stat.connected ? "connected" : "disconnected");
    }
}

static void send_passthrough(uint8_t command)
{
    if (!s_avrc_controller_connected) {
        ESP_LOGW(TAG, "button ignored: AVRCP controller is not connected");
        return;
    }
    const uint8_t pressed_label = s_avrc_transaction_label++ & 0x0f;
    const uint8_t released_label = s_avrc_transaction_label++ & 0x0f;
    esp_err_t result = esp_avrc_ct_send_passthrough_cmd(
        pressed_label, command, ESP_AVRC_PT_CMD_STATE_PRESSED);
    if (result == ESP_OK) {
        result = esp_avrc_ct_send_passthrough_cmd(
            released_label, command, ESP_AVRC_PT_CMD_STATE_RELEASED);
    }
    if (result != ESP_OK) ESP_LOGW(TAG, "AVRCP button command failed: %s", esp_err_to_name(result));
}

static void button_action(button_action_t action)
{
    switch (action) {
        case button_action_t::previous:
            send_passthrough(ESP_AVRC_PT_CMD_BACKWARD);
            break;
        case button_action_t::play_pause:
            send_passthrough(s_playing ? ESP_AVRC_PT_CMD_PAUSE : ESP_AVRC_PT_CMD_PLAY);
            break;
        case button_action_t::next:
            send_passthrough(ESP_AVRC_PT_CMD_FORWARD);
            break;
        case button_action_t::reverb_next: {
            const uint8_t level = (speaker_spatializer_get_reverb_level() + 1) % 4;
            speaker_spatializer_set_reverb_level(level);
            spectrum_display_set_reverb(level);
            ESP_LOGI(TAG, "reverb level=%u", (unsigned)level);
            break;
        }
        case button_action_t::volume_down:
            apply_absolute_volume(s_absolute_volume > 6 ? s_absolute_volume - 6 : 0, true);
            break;
        case button_action_t::volume_up:
            apply_absolute_volume(s_absolute_volume < 121 ? s_absolute_volume + 6 : 127, true);
            break;
    }
}
...
static void init_bluetooth(void)
{
    ...
    ESP_ERROR_CHECK(esp_avrc_ct_register_callback(avrc_controller_callback));
...
extern "C" void app_main(void)
{
    ...
    if (display_result != ESP_OK) {
        ESP_LOGW(TAG, "ST7735 unavailable: %s; audio will continue", esp_err_to_name(display_result));
    }
    xTaskCreatePinnedToCore(decoder_task, "a2dp_decoder", DECODER_STACK, nullptr,
                            DECODER_PRIORITY, &s_decoder_task, DECODER_CORE);
    init_bluetooth();
    const esp_err_t button_result = buttons_init(button_action);
    if (button_result != ESP_OK) {
        ESP_LOGW(TAG, "buttons unavailable: %s; audio will continue", esp_err_to_name(button_result));
    }
    ESP_LOGI(TAG, "firmware=%s", FIRMWARE_REV);
    ESP_LOGI(TAG, "ready: LDAC/aptX HD/Classic/AAC/SBC -> PCM5102 + ST7735 FFT + 4 buttons");
